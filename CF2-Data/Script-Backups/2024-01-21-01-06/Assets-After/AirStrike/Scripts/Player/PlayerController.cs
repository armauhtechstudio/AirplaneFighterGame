...
public class PlayerController : MonoBehaviour
{
	...
	void DesktopController ()
	{
		...
		flight.AxisControl (new Vector2 (ControlFreak2.CF2Input.GetAxis ("Mouse X"), ControlFreak2.CF2Input.GetAxis ("Mouse Y")));
		if (SimpleControl) {
			flight.DirectVelocity = false;
			flight.TurnControl (ControlFreak2.CF2Input.GetAxis ("Mouse X"));
		} else {
			flight.DirectVelocity = directVelBack;	
		}

		flight.TurnControl (ControlFreak2.CF2Input.GetAxis ("Horizontal"));
		flight.SpeedUp (ControlFreak2.CF2Input.GetAxis ("Vertical"));
