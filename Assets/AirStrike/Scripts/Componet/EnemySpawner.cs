...
public class EnemySpawner : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (gos.Length < enemyCount && Time.time > timetemp + timeSpawn) {
			...
			timetemp = Time.time;
			GameObject obj = Instantiate (Objectman [indexSpawn], transform.position + new Vector3 (Random.Range (-radius, radius), 0, Random.Range (-radius, radius)), Quaternion.identity);
			obj.tag = Tag;
			indexSpawn = 0;
		}
	}
